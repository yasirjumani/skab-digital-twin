...
def run():
    ...
    # Three-way split: calibration | validation | test
    n = len(df)
    val_start  = CALIBRATION_ROWS
    val_end    = CALIBRATION_ROWS + (n - CALIBRATION_ROWS) // 2
    # val rows used ONLY for threshold selection; test rows for final reporting
    ...
    t_base    = best_threshold(scores_val_base, y_val)   # threshold chosen on VAL
    base_pred = (scores_test_base < t_base).astype(int)  # applied to TEST
    ...
    t_roll    = best_threshold(scores_val_roll, y_val)    # threshold chosen on VAL
    roll_pred = (scores_test_roll < t_roll).astype(int)   # applied to TEST
